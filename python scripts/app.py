...
<div id="input-area">
  ...
  <button id="btn" onclick="send()">Envoyer ✈️</button>
</div>
...
(function init() {
  ...
  d.innerHTML = '<div class="avatar bot">🤖</div>' +
    '<div>' +
      '<div class="bubble">Bonjour ! Je suis l\'assistant Puls-Events. ' +
      ...
      '<div class="chips">' +
        ...
        '<span class="chip" onclick="send(this.innerText)">👨‍👩‍👧 Activités enfants</span>' +
...
function send(text) {
  ...
  xhr.onload = function() {
    ...
    if (xhr.status === 200) {
      try {
        ...
        addMsg('bot', '❌ Erreur lors du décodage de la réponse.', null);
      }
    } else {
      addMsg('bot', '❌ Erreur serveur (' + xhr.status + '). Vérifiez que app.py tourne.', null);
    }
    isReady = true;
    btn.disabled = false;
    setStatus(false);
  ...
  xhr.onerror = function() {
    removeTyping();
    addMsg('bot', '❌ Impossible de contacter le serveur. Vérifiez que <code>python scripts/app.py</code> est lancé.', null);
    isReady = true;
    btn.disabled = false;
    setStatus(false);
  };
  ...
  xhr.ontimeout = function() {
    removeTyping();
    addMsg('bot', '⏱️ Délai dépassé (30s). Le LLM Mistral met trop de temps à répondre.', null);
    isReady = true;
    btn.disabled = false;
    setStatus(false);
  };
...
def init_rag():
    ...
    if not (INDEX_DIR / "index.faiss").exists():
        print(f"❌ Index FAISS introuvable : {INDEX_DIR}")
